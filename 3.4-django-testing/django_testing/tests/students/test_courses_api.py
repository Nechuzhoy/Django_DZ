import pytest
from rest_framework.test import APIClient  # клиент который сможет отправлять запросы API сервис
...
@pytest.mark.django_db
def test_get_course_filters_id(client, course_factory):
    course = course_factory(_quantity=1)
...
@pytest.mark.django_db
def test_get_course_filters_name(client, course_factory):
    course = course_factory(_quantity=1)
...
def test_create_course(client):
    ...
    response = client.post('/api/v1/courses/', data={'name': 'create_course'})
